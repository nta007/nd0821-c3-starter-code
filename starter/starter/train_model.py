...
from starter.const import cat_features, stored_model_filepath, metrics_output_filepath
    ...
    for feat in cat_features:
        for col in test_df[feat].unique():
            ...
            X__test, y__test, _, _ = process_data(
                _df, categorical_features=cat_features, label="salary", training=False, encoder=encoder, lb=lb
            )

            preds = model.inference(stored_model, X__test)
            precision, recall, fbeta = model.compute_model_metrics(y__test, preds)
            metric = {
                "feature": feat,
                "value": col,
                "precision": precision,
